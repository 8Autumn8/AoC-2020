...
class Main {
  ...
  private static HashMap<String,String> allergins = new HashMap<String,String>();
  ...
  public static void main(String[] args) {
    ...
    System.out.println(allergins);

    List<String> alphabet = new ArrayList<String>();
    for (String key: allergins.keySet()){
      alphabet.add(key);
    }
    Collections.sort(alphabet);
    String partTwo = "";
    for (String x : alphabet){
      partTwo += allergins.get(x) + ",";
    }
  ...
  public static String comareAllValues(List<String> x){
    return "";
  }
  ...
  public static List<String> checkEqualValues(List<String> original, List<String> compare){
    //System.out.println(" ************** ");
    ...
    for (int i = 0; i < original.size(); i++){
      for (int j = 0; j < compare.size();j++){
        ...
        if (original.get(i).equals(compare.get(j))){
          ...
          similarValues.add(original.get(i));
        }
        //System.out.println("they are not equal");
      }
    }
      ...
      if (keyCompare != keyMain && !keyMain.isEmpty() && !keyCompare.isEmpty()){
        ...
        if (!x.equals("")){
          ...
          for (int l = 0; l < map.size(); l++){
            List<List<String>> newCompare = new ArrayList<List<String>>(map.get(l));
            List<String> newKeyCompare = newCompare.get(0);
            List<String> newvaluesCompare = newCompare.get(1); 

            if (newKeyCompare.contains(x)){
              originalValue = checkEqualValues(originalValue, newvaluesCompare);
            }           
          }
          if (originalValue.size() == 1){
            String equal = originalValue.get(0);
            allergins.put(x,equal);
            if(!equal.equals("")){
  ...
  public static List<List<List<String>>> multipleValues(List<String> keysToRemove, List<String> valuesToRemove){
    ...
    for (List<List<String>> x : map){
      ...
      for (int i = 0; i < keysToRemove.size();i++){
        allergins.put(keysToRemove.get(i),valuesToRemove.get(i));
      }
      List<String> keys = new ArrayList<String>(keyMain);
      List<String> values = new ArrayList<String>(valuesMain);
